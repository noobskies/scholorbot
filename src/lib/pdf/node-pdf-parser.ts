...
async function parseNodePdf(
  ...
  try {
    ...
    const numPages = max <= 0 ? pdfDocument.numPages : Math.min(max, pdfDocument.numPages);
    ...
    return {
      ...
      metadata: info.metadata ? (info.metadata as unknown as Record<string, unknown>) : {},
      version: options.version || '1.0',
    };
  } catch (error) {
    console.error('Error parsing PDF with Node.js:', error);
    throw new Error(`Failed to parse PDF in Node.js: ${error instanceof Error ? error.message : String(error)}`);
  }
}
 ...
 */
async function defaultRenderPage(pageData: any): Promise<string> {
